...
int main()
{
    ...
    * if the file rec.txt already exists then it open that file in read write mode
    */
    fp = fopen("rec.txt","rb+");
    if(fp == NULL)
    {
    ...
    while(1)
    {
        ...
        case '4':
            ...
            while(another == 'y')
            {
                ...
                fclose(fp);
                fclose(ft);
                remove("rec.txt"); /// remove the orginal file
                rename("temp.txt","rec.txt"); /// rename the temp file to original file name
                fp = fopen("rec.txt", "rb+");
